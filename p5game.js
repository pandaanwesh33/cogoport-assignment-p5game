...
let bricks = [];
let score = 0;
let life = 3; // keep tracks of no. of life
...
function initializeGame() {
  ...
  for (let row = 0; row < num_rows; row++) {
    for (let col = 0; col < num_bricks_per_row; col++) {
      bricks.push({
        x: col * (brick_width + 5) + 20, // added 5 for spacing between bricks
...
function draw() {
  ...
  if (ball_y + (ball_diameter / 2) > height) {
    ball_dy = -ball_dy;
    life--; // Decrease life when the ball touches the lower wall
    
    if (life === 0) {
      console.log("Game Over"); // Print "Game Over" in the console if life becomes 0
      noLoop(); // it stops the game
    } else {
